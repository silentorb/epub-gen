...
(function() {
  ...
  EPub = class EPub {
    ...
    generate() {
      ...
      this.id = uuid();
      this.uuid = path.resolve(this.options.tempDir, (process.env.PRESERVE_TEMP_DIR ? 'default' : this.id));
      ...
      this.options.content = _.map(this.options.content, function(content, index) {
        ...
        $("img").each(function(index, elem) {
          ...
          return $(elem).attr("src", `images/${id}`);
        });
        content.data = $.xml();
        return content;
      });
    ...
    generateTempFile() {
      ...
      if (process.env.PRESERVE_TEMP_DIR && fs.existsSync(this.options.tempDir)) {
        rimraf.sync(this.uuid);
      }
